...
def train_cephalo(name, landmarks, load=False, startEpoch=0, batched=False, fold=3, num_folds=4, fold_size=100, graphical=False,iterations=10, avg_labels=False,rms=False):
    ...
    splits, datasets, dataloaders, annos = CephaloXrayData.get_folded(landmarks,fold=fold,num_folds=num_folds,fold_size=fold_size,batchsize=batchsize)
        ...
        for phase in ['train', 'val']:
            ...
            for i in range(len(dataloaders[phase])):
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    error = loss.detach().sum(dim=2).sqrt()
                    errors.append(error)
                    # doc_errors.append(F.mse_loss(junior_labels,reduction='none').sum(dim=2).sqrt())
        ...
        if test==1:
            ...
            for fold in range(1):
                for i in pnts:
                    (name, cephalo_pnt) = i
                    print(f"Running fold {fold}, point {name}, Cephalo:{cephalo_pnt}")
                    train_cephalo(f"big_cephalo_{cephalo_pnt}_{fold}", [cephalo_pnt], batched=True, fold=fold, num_folds=4, fold_size=2682, iterations=10, load=True ,avg_labels=False)
